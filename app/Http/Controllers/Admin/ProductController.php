...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //     $limitedName = implode(' ', array_slice(explode(' ', $product->name), 0, 8));
        //     $slug = Str::slug($limitedName);
        //     $finalSlug = $slug . '-' . (1000 + $product->id);
        //     $product->update(['slug' => $finalSlug]);
        // }
    ...
    public function store(Request $request)
    {
        ...
        $limitedName = implode(' ', array_slice(explode(' ', $save_data->name), 0, 8));
        $slug = Str::slug($limitedName);
        $finalSlug = $slug . '-' . (1000 + $save_data->id);
        $save_data->update(['slug' => $finalSlug]);
        $save_data->update(['product_code' => 1000 + $save_data->id]);
    ...
    public function update(Request $request)
    {
        ...
        $limitedName = implode(' ', array_slice(explode(' ', $update_data->name), 0, 8));
        $slug = Str::slug($limitedName);
        $finalSlug = $slug . '-' . (1000 + $update_data->id);
        $update_data->update(['slug' => $finalSlug]);
        $update_data->update(['product_code' => 1000 + $update_data->id]);
